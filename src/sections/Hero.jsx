...
const Hero = () => {
  ...
  return (
    <section id="Home" className="w-full p-2 flex xl:flex-row min-h-screen flex-col justify-center gap-10 max-container">
      <div className="flex relative xl:w-2/5 flex-col justify-center items-start w-full max-xl:padding-x pt-28">
        ...
        <div className="flex sm:gap-6 gap-4 absolute -bottom-[5%] sm:left-[10%] max-sm:px-6">
          {shoes.map((shoe) => (
            <div key={shoe.thumbnail}>
              <ShoeCard imageURL = {shoe} changeBigShoeImage = {(shoe) => setBigShoeImg(shoe)} bigShoeImage = {bigShoeImage}/>
            </div>
          ))}
        </div>
      </div>
